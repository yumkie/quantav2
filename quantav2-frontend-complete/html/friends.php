    ...
    <main>
        <div class="friend_container">
            ...
            <div class="result_container">
                ...
                <?php } ?>
             <?php
                    $select = "SELECT * FROM Friend_requests WHERE user_inviter = '$id'";
                    $result = mysqli_query($link, $select);
                    while($user1 = mysqli_fetch_array($result)) {
                        $invited = $user1['user_invited'];
                        $select = "SELECT * FROM Users WHERE UserID = '$invited'";
                        $result = mysqli_query($link, $select);
                        $user = mysqli_fetch_assoc($result);
                        ?>
                <nav class="sent">
                    ...
                    <form action='../php/delete_sent.php' method='POST'>
                        <input type='submit' name='<?php echo $user1['Request_id']; ?>' value=''>
                    </form>
                </nav>
                <?php } ?>
                <?php
                    $select = "SELECT * FROM Friend_requests WHERE user_invited = '$id'";
                    $result = mysqli_query($link, $select);
                    while($user1 = mysqli_fetch_array($result)) {
                        $inviter = $user1['user_inviter'];
                        $_SESSION['inv'] = $inviter;
                        $inv = $_SESSION['inv'];
                        $select = "SELECT * FROM Users WHERE UserID = '$inviter'";
                        $result = mysqli_query($link, $select);
                        $user = mysqli_fetch_assoc($result);
                ?>
