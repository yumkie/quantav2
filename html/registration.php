        ...
        <div class="registration_container">
            ...
            <?php
                ...
                if(isset($_POST['sub'])) {
                    ...
                    $select = "SELECT `UserID`, `Login`, `Password`, `Email` FROM `Users` WHERE Email='$email' AND LOgin='$username'"; //Выбор данных из бд
                    $res = mysqli_query($link, $select);
                    $user = mysqli_fetch_assoc($res);
                    if ($password == $confirm_password) {
                        if (!isset($user)) {
                            $_SESSION['auth'] = true; //индикация входа пользователя в аккаунт
                            $_SESSION['email'] = $email;  //сохранение id пользователя в сессии
                            ...
                            header("Location: registration2.php"); //редирект при успешной авторизации
                            die();
                    }else {
